...
export async function GET(request: NextRequest) {
  ...
  // Use forwarded headers or NEXT_PUBLIC_URL for redirects in production
  // Railway/proxies provide x-forwarded-host with the public domain
  // request.nextUrl.origin returns container's internal address (0.0.0.0:8080) which breaks redirects
  const forwardedHost = request.headers.get('x-forwarded-host') || request.headers.get('host')
  const forwardedProto = request.headers.get('x-forwarded-proto') || 'https'
  const baseUrl = forwardedHost
    ? `${forwardedProto}://${forwardedHost}`
    : process.env.NEXT_PUBLIC_URL || request.nextUrl.origin || 'http://localhost:3000'
