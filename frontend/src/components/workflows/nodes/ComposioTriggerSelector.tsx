...
import { useComposioAppsWithTriggers, useComposioAppTriggers, ComposioTriggerType } from '@/hooks/composio/use-composio-triggers';
...
interface ComposioTriggerSelectorProps {
    open: boolean;
...
export const ComposioTriggerSelector: React.FC<ComposioTriggerSelectorProps> = ({
    open,
    onOpenChange,
    agentId,
    onSave,
    ...
    const { data: allProfiles } = useComposioProfiles();
    ...
    useEffect(() => {
        if (open && initialConfig && initialConfig.appSlug && !selectedApp) {
            // We need to find the app object from appsData to set selectedApp
            // But appsData might not be loaded yet.
            // We can optimistically set selectedApp if we have enough info, or wait for appsData.
            if (appsData?.items) {
                const app = appsData.items.find(a => a.slug === initialConfig.appSlug);
                if (app) {
                    setSelectedApp(app);
                    if (initialConfig.triggerSlug) {
                        // We need triggersData to set selectedTrigger
                        // This will happen in next render when triggersData loads
                    }
                }
            }
        }
    }, [open, initialConfig, appsData, selectedApp]);

    useEffect(() => {
        if (open && initialConfig && selectedApp && triggersData?.items && !selectedTrigger) {
            if (initialConfig.triggerSlug) {
                const trigger = triggersData.items.find(t => t.slug === initialConfig.triggerSlug);
                if (trigger) {
                    setSelectedTrigger(trigger);
                    if (initialConfig.config) setConfig(initialConfig.config);
                    if (initialConfig.profileId) setProfileId(initialConfig.profileId);
                    setStep('config');
                }
            }
        }
    }, [open, initialConfig, selectedApp, triggersData, selectedTrigger]);
    ...
    const handleSave = () => {
        if (!selectedApp || !selectedTrigger || !profileId) {
                                    ...
                                    <div className="p-6 border-b flex items-center justify-between">
                                        <div>
                                            ...
                                            <p className="text-sm text-muted-foreground">
                                                Choose an app to monitor for events
                                            </p>
                                        </div>
                                ...
                                <div className="h-full flex flex-col">
                                    ...
                                    <div className="p-6 border-t bg-muted/20">
                                        <div className="flex justify-end gap-3">
                                            ...
                                            <Button onClick={handleSave} disabled={!profileId}>
                                                Save Trigger
                                            </Button>
                                        </div>
                                    </div>
