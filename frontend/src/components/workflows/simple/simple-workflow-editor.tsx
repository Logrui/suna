...
import { useAgentTools, useAgent } from '@/hooks/agents';
...
export function SimpleWorkflowEditor({ agentId, workflowId }: SimpleWorkflowEditorProps) {
    ...
    // Fetch global tools metadata for rich display names
    const { data: toolsMetadataResponse } = useToolsMetadata();
    const toolsMetadata = toolsMetadataResponse?.tools;
    ...
    // Handle workflow name change - update local state and persist to database
    const handleNameChange = useCallback(async (newName: string) => {
        setWorkflowName(newName);

        // Persist to database if we're editing an existing workflow
    ...
    }, [isEditing, agentId, workflowId, updateWorkflowMutation]);
