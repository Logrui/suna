...
export function ConditionalGroup({
    ...
    return (
        <div ref={setNodeRef} style={style} className={cn("relative group/condition", isDragging && "opacity-50")}>
            {/* Background Glow Effect for the whole group */}
            <div className="absolute -inset-1 bg-gradient-to-r from-blue-500/5 to-indigo-500/5 rounded-2xl blur opacity-0 group-hover/condition:opacity-100 transition duration-500" />

            <div className="relative space-y-4 bg-background/40 backdrop-blur-md border border-border/50 rounded-2xl p-4 transition-all duration-300 group-hover/condition:border-blue-500/20 group-hover/condition:shadow-xl dark:bg-zinc-900/40">
                {/* Grain Overlay */}
                <div className="pointer-events-none absolute inset-0 opacity-[0.02] mix-blend-overlay rounded-2xl"
                ...
                <div className="absolute top-4 right-4 z-10 opacity-0 group-hover/condition:opacity-100 transition-all">
                    <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => {
                            const firstStep = conditionSteps[0];
                            if (firstStep) onEdit(firstStep);
                        }}
                        ...
                        return (
                            <div key={step.id} className="relative group/tab">
                                <Button
                                    ...
                                    className={cn(
                                        "h-9 px-4 rounded-xl border transition-all duration-300 text-xs font-medium tracking-tight",
                                        isActive
                                            ? "bg-blue-500 text-white border-blue-400 shadow-[0_0_15px_-3px_rgba(59,130,246,0.4)]"
                                            : "bg-background/50 border-border/50 text-muted-foreground hover:bg-background hover:border-blue-500/30 hover:text-blue-500"
                                    )}
                                >
                                    <div className={cn(
                                        "flex items-center justify-center w-5 h-5 rounded-md text-[10px] font-bold mr-2",
                                        isActive ? "bg-white/20" : "bg-muted text-muted-foreground"
                                    )}>
                                        {letter}
                                    </div>
                                    <span className="uppercase tracking-widest text-[10px]">{conditionType}</span>
                                    {step.hasIssues && (
                                        <AlertTriangle className="h-3 w-3 text-red-500 ml-2" />
                                    )}
                                </Button>
                ...
                {activeStep && (
                    <div className="relative bg-background/60 backdrop-blur-sm rounded-2xl p-5 border border-border/50 shadow-inner">
