...
import { CATEGORY_DEFINITIONS, generateAvailableStepTypes } from '../../shared/workflow-definitions';
...
export function useWorkflowSteps({
    ...
    // Uses toolsMetadata from backend for rich display names and descriptions
    const getAvailableStepTypes = useCallback((): StepType[] => {
        return generateAvailableStepTypes(agentTools, toolsMetadata).map(step => ({
            id: step.id,
            name: step.name,
            description: step.description,
            icon: typeof step.icon === 'function' ? step.icon.displayName || step.icon.name || 'FileText' : 'FileText',
            iconUrl: step.iconUrl,
            group: step.group,
            groupIconUrl: step.groupIconUrl,
            category: step.category,
            config: step.config
        }));
    }, [agentTools, toolsMetadata]);
