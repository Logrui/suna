...
export function FileViewerView({
  ...
  useEffect(() => {
    ...
    // Handle errors - show immediately if error is definitive (404) or after 5 retries
    const isDefinitiveError = cachedFileError?.message?.includes('404') ||
      cachedFileError?.message?.toLowerCase().includes('not found');
    const showError = cachedFileError && !isCachedFileLoading && (isDefinitiveError || fileRetryAttempt >= 3);
