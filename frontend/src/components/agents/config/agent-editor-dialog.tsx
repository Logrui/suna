...
import { AgentPermissionsConfig } from './AgentPermissionsConfig';
...
interface AgentEditorDialogProps {
    ...
    permissionSettings?: ToolPermissionSettings;
    onSave?: (data: {
        name: string;
...
export function AgentEditorDialog({
    ...
    const [currentPermissionSettings, setCurrentPermissionSettings] = useState<ToolPermissionSettings | undefined>(permissionSettings);
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            setCurrentPermissionSettings(permissionSettings);

            // Extract hue from current background color
            const currentHue = hexToHue(currentBackgroundColor);
            setHue(currentHue);
        }
    }, [isOpen, agentName, currentIconName, currentBackgroundColor, permissionSettings]);
    ...
    const handleSave = () => {
        ...
        if (onSave) {
            onSave({
                ...
                iconColor,
                backgroundColor,
                permissionSettings: currentPermissionSettings
            });
            onClose();
        }
    };
                ...
                <div className="flex-1 min-h-0 overflow-auto px-6 pb-4">
                    <Tabs defaultValue="appearance" className="w-full h-full flex flex-col">
                        <TabsList className="w-full justify-start border-b rounded-none px-0 h-auto bg-transparent">
                            <TabsTrigger value="appearance" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent px-4 py-2">Appearance</TabsTrigger>
                            <TabsTrigger value="permissions" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent px-4 py-2">Permissions</TabsTrigger>
                        </TabsList>
                        ...
                        <TabsContent value="permissions" className="flex-1 mt-4 overflow-y-auto">
                            <AgentPermissionsConfig
                                permissionSettings={currentPermissionSettings}
                                onUpdate={setCurrentPermissionSettings}
                            />
                        </TabsContent>
                    </Tabs>
                </div>
