...
class MCPToolExecutor:
    ...
    async def execute_tool(self, tool_name: str, args: Dict[str, Any]) -> Any:
        try:
            ...
            # This avoids fallback overhead for servers with known transport
            detected = self.mcp_config.get("detectedTransport", "")
    ...
    async def _execute_sse_tool(self, tool_name: str, args: Dict[str, Any]) -> Any:
        ...
        try:
            async with asyncio.timeout(30):
                async with streamablehttp_client(url, headers=headers) as (read, write, _):
                    async with ClientSession(read, write) as session:
                        ...
                        result = await session.call_tool(tool_name, arguments=args)
                        content = self._extract_result_content(result)
                        logger.info(f"✅ [MCP EXEC] Executed {tool_name} via Streamable HTTP fallback")
                        return ToolResult(success=True, output=str(content))
        except asyncio.TimeoutError:
            logger.error(f"❌ [MCP EXEC] Streamable HTTP fallback timeout for {tool_name}")
            return ToolResult(
                success=False,
                output=f"Tool execution timeout after 30 seconds (both SSE and Streamable HTTP)"
            )
            ...
            return ToolResult(
                success=False,
                output=f"Failed to execute tool (SSE: {sse_error}, Streamable HTTP: {http_err})"
            )
