...
from core.workflows.models import Workflow, CreateWorkflowRequest, UpdateWorkflowRequest
...
async def list_all_workflows(
    ...
    """List all workflows for the current user across all agents"""
    db = DBConnection()
    ...
    # Query workflows with agent info, filtered by user's agents
    # Note: agents table uses account_id (not user_id) to link to users
    response = await supabase.table("agent_workflows") \
        .select("*, agents!inner(account_id, name)") \
        .eq("agents.account_id", user_id) \
        .order("updated_at", desc=True) \
        .range(offset, offset + limit - 1) \
        .execute()
    
    # Return just the workflow data (not the nested agent info)
    workflows = []
    for item in response.data:
        workflow = {k: v for k, v in item.items() if k != 'agents'}
...
async def execute_workflow(
    ...
    elif mode in ('advanced', 'advanced-legacy'):
    ...
    async def event_generator():
        ...
        try:
            ...
            while True:
                ...
                # Send keepalive ping to prevent socket timeout
                now = time.time()
                if now - last_ping > 15.0:
                    try:
                        await pubsub.ping()
                        last_ping = now
                        # Also yield a comment to keep HTTP connection alive
                        yield ": ping\n\n" 
                    except Exception:
                        pass
