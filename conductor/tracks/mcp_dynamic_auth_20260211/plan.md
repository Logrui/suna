...
- [x] **Task 4.1**: Create a local test MCP server (or mock) that requires specific headers.
    - [x] Verified via `backend/core/test_harness/mcp_lab/runner.py`.
...
## Phase 5: Refinement ✅
- [x] **Task 5.1**: Ensure error handling for timeouts or unreachable OAuth metadata endpoints is user-friendly.
- [x] **Task 5.2**: Clean up logging (remove sensitive tokens from logs).
    - [x] Sanitization (`***`) implemented in `api.py`.
