...
# Fork Syncing Operating Procedure - High Level Overview

Step 1: Sync branch main with upstream. Pull main to local dev environment

Step 2: Create backup dev branch using backup-dev.ps1

Step 3: Switch back to dev branch and ensure it is the most up to date fork branch

Step 3B: Identify Merge Base (last commit when main and dev were identical) using git log -1 $(git merge-base main dev) --oneline

Step 3C: Use commit hash - typically 184b7c85006ff0473898a30d557433c8c95acd68 (hash of original fork) in further steps

Step 3D: Get current commit hash of dev branch and main branch

Step 4: Use original-fork-diff.ps1 and diff the difference between dev and the original timepoint we branched from

File Lists Needed:
-"Which files do we want to to 100% include from our dev branch after git reset hard?" ----> 'files modified in dev since merge-base'
-"What files were deleted by Main compared to merge-base?" ----> Mirror these deletions in Dev branch (safe to delete)
-"What files has Main added compared to merge-base?" ------> Mirror these newe files into Dev branch (safe to add)
-"What files has Main modified compared to merge-base?" -----> manual review needed to sync
-"What files overlap in modifications between Main/Dev compared to merge base" ----> manual review needed to sync
-"What files does Dev have that Main does not have?" -----> Keep ours - do not delete or manually restore after git reset hard
-"What config files do we explicitely want to keep-ours from Dev" ----> Keep ours

Step 5: Use analyze-diff.ps1 for automated analysis between dev and main - not realy useful information - except for Deleted Files - those should be cherry picked added back after sync

Step 6: Use actual-diff.ps1 to subtract the differences between ADDED, MODIFIED, and DELETED to get the real list of files that were recently ADDED, MODIFIED, OR DELETED by Kortix Team
...
## Historical Sync Logs Below


## November 9th 2025
-Need to pull any and all frontend/backend updates from Main (the newest sync with upstream) - Frontend is currently critically broken in our fork. We need to test if the newest updates fixes bugs related to minified code updates 
-Reset hard and likely proceed to selectively restore certain files
